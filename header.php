...
<div id="page" class="site">
    ...
    <?php 
        if (!is_single() && !is_search() && !is_404()) {
            get_template_part( 'template-parts/layout/hero-section' );
        } elseif (is_search()) {
            get_template_part( 'template-parts/layout/hero-section', 'search' );
        }
    ?>
	<div id="content" class="site-content <?php root_site_content_classes() ?>">
        ...
        if ( is_single() && in_array( 'post', $ad_visible ) ) {
            $show_ad = do_show_ad(
                ...
                isset( $ad_options['r_before_site_content_category_include'] ) ? $ad_options['r_before_site_content_category_include'] : array(),
