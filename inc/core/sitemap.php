 ...
 * Class Sitemap
 *
 * 1.0.3    2021-10-08      Fix get_the_title on pages
 *                          Add attr taxonomies and taxonomies_show_posts
        ...
        if ( ! empty( $pages ) ) {
            ...
            foreach ( $pages as $page ) {
                if ( empty( $pages_exclude ) || ( ! empty( $pages_exclude ) && ! in_array( $page->ID, $pages_exclude ) ) ) {
                    $out .= '<li><a href="' . get_the_permalink( $page->ID ) . '">' . get_the_title( $page->ID ) . '</a>';
